...
def observation_to_tensor(obs, action_for_observation=None, total_number_of_tests=None):
    ...
    test_covers_mutant_tensor = torch.tensor([obs.test_covers_mutant], dtype=torch.float32)
    if action_for_observation is None:
        return torch.cat((test_index_tensor, test_sequence_precise_tensor, mutant_operator_tensor, test_covers_mutant_tensor))
