...
class Prioritizer:
    ...
    '''
    def __init__(self, tests_folder_path, mutants_path, sut_name, plot_delta, average_delta, results_file_name='results.json', best_params_file_name='best_params.json'):
...
if __name__ == '__main__':
    ...
    if os.path.exists('experiments/results.json'):
        ...
        experiment_ids = [int(re.search(r'results_(\d+)\.json', f).group(1)) for f in existing_files if re.search(r'results_(\d+)\.json', f)]
    ...
    if os.path.exists('experiments/best_params.json'):
        ...
        best_params_ids = [int(re.search(r'best_params_(\d+)\.json', f).group(1)) for f in existing_files if re.search(r'best_params_(\d+)\.json', f)]
