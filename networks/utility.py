...
def observation_to_tensor(obs, action_for_observation=None, total_number_of_tests=None):
    ...
    variance_tensor = torch.tensor([obs.variance], dtype=torch.float32)
    if action_for_observation is None:
        return torch.cat((test_index_tensor, test_sequence_precise_tensor, mutant_operator_tensor, variance_tensor))
