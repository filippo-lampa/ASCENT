...
abstract contract StrategySwapAnd is IStrategy, ReentrancyGuard, Ownable {
     ...
     */
    constructor(address initialOwner, IERC20 iAsset, IStrategy iUnderlyingStrategy, IOracle iOracle) Ownable() {
        if (
    ...
    function deploy(uint256 amount) external override nonReentrant returns (uint256 amountUsed) {
        ...
        uint256 minAmountOut = (calculatedAmountOut * (PERCENTAGE_PRECISION - _maxSlippage)) / PERCENTAGE_PRECISION;
     ...
     * @inheritdoc IStrategy
     */
    function undeploy(uint256 amount) external override nonReentrant returns (uint256 undeployedAmount_) {
        if (amount == 0) revert InvalidAmount();
    ...
    function _totalAssets() internal view returns (uint256 amount_) {
        ...
        amount_ = _convertFromUnderlying(IOracle.PriceOptions({maxAge: 0, maxConf: 0}), underlyingAssets);
        return amount_;
    }
